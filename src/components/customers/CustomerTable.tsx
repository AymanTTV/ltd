...
import { Eye, Edit, Trash2, FileText, FileCheck } from 'lucide-react'
...
export default function CustomerTable({
  ...
  const columns = [
      ...
      cell: ({ row }: any) => (
        <div>
          ...
          {row.original.badgeNumber && (
            <div className="text-xs text-gray-500">{row.original.badgeNumber}</div>
          )}
        </div>
      ),
    },
    ...
    },
    {
      ...
      accessorKey: 'mobile',
      cell: ({ row }: any) => (
        <a href={`tel:${row.original.mobile}`} className="text-primary underline">
          {row.original.mobile}
        </a>
      ),
    },
    {
      ...
      accessorKey: 'email',
      cell: ({ row }: any) => (
        <a href={`mailto:${row.original.email}`} className="text-primary underline">
          {row.original.email}
        </a>
      ),
    },
    {
      ...
      cell: ({ row }: any) => format(row.original.createdAt, 'dd/MM/yyyy'),
    },
    {
      header: 'License Type',
    ...
    },
    {
      ...
      cell: ({ row }: any) => (
        <div className="flex space-x-2">
          {can('customers', 'view') && (
            <button onClick={() => onView(row.original)} title="View Member">
